...
export function getObjectSizeWithStroke(object: fabric.Path) {
  ...
  return new fabric.Point((object.width as number) + stroke.x, (object.height as number) + stroke.y)
}
...
export const actionHandler: fabric.Control['actionHandler'] = function (
  ...
  const pathObj = transform.target as fabric.Path,
    ...
    mouseLocalPosition = pathObj.toLocalPoint(new fabric.Point(x, y), 'center', 'center'),
    pathBaseSize = getObjectSizeWithStroke(pathObj),
    size = pathObj._getTransformedDimensions(0, 0),
    finalPointPosition = {
      x: (mouseLocalPosition.x * pathBaseSize.x) / size.x + pathObj.pathOffset.x,
  ...
  if (
    ...
    const newPath = calculateArrowSlidePath(path, point1[1], point1[2], point2[1], point2[2])
...
export function anchorWrapper(anchorIndex: number, fn: fabric.Control['actionHandler']) {
  return function (eventData: MouseEvent, transform: fabric.Transform, x: number, y: number) {
    ...
    const newX = (paths[anchorIndex][1] - fabricObject.pathOffset.x) / pathBaseSize.x
    const newY = (paths[anchorIndex][2] - fabricObject.pathOffset.y) / pathBaseSize.y
